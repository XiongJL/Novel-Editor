...
export default function MapSidebar({ novelId, theme, onSelectMap, activeMapId }: MapSidebarProps) {
    ...
    useEffect(() => {
        ...
        window.addEventListener('map-assets-updated', handleRefresh as EventListener);
        return () => window.removeEventListener('map-assets-updated', handleRefresh as EventListener);
    }, [loadMaps, novelId]);
            ...
            <div className="p-2 space-y-2 flex-1 overflow-y-auto">
                {maps.length === 0 ? (
                    <div className={clsx('text-center py-12', isDark ? 'text-neutral-600' : 'text-neutral-400')}>
                        ...
                        <p className="text-xs">{t('map.noMaps')}</p>
                        <p className="text-[10px] mt-1 opacity-60">{t('map.noMapsHint')}</p>
                    </div>
                ) : (
                    maps.map((map) => {
                                    ...
                                    {isRenaming ? (
                                        ...
                                        <>
                                            ...
                                            <div className={clsx('text-[10px] mt-0.5', isDark ? 'text-neutral-500' : 'text-neutral-400')}>
                                                ...
                                                {map.background ? ` · ${t('map.hasBackground', '已有底图')}` : ''}
                                            </div>
                                        </>
                                    )}
                                </div>
